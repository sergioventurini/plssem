...
program plssem_mi
    ...
    if (`"`options'"' != "") {
      if (`"`options'"' == "midisplay") {
        ...
        Display_mi, `options' `nostructural'
      }
    }
    else {
      Display_mi, `old_options' `nostructural'
    }
    exit
	}
  ...
  `version' Estimate_mi `0'  // version is not necessary
end

program Estimate_mi, eclass
	version 15.1
  ...
  quietly plssem `anything', `options'
  ...
  if ("`midisplay'" == "") {
    ...
    mi estimate, cmdok noheader notable `mioptions': plssem `anything', `options'
    /* End of multiple imputation analysis */
    
    /* Display results */
    Display_mi, wscheme(`wscheme') init(`init') `structural' ///
    ...
    /* Multiple imputation analysis */
    mi estimate, cmdok `mioptions': plssem `anything', `options'
    /* End of multiple imputation analysis */
  }
