...
program Estimate, eclass byable(recall) properties(mi)
  version 15.1
  ...
  /* Options:
     ...
     convcrit(string)               --> convergence criterion (either 'relative'
                                        or 'square')
...
//    matrix colnames `strvif' = `lv_regest_all'
  ...
  if (("`display'" == "") & ("`group'" == "")) {  // this saves some time when used with 'group' 
    ...
    Display, `nostructural' digits(`digits') `header' `meastable'  ///
      `nodiscrim' `structtable' `loadpval' `stats' `corrind' `corrlv' ///
      `crossload' cutoff(`cutoff') binary(`binary') `rawsum'
  }
