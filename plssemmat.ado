...
program Estimate_mat, eclass byable(recall) properties(mi)
  version 15.1
    ...
    mata: st_local("i`j'", ///
      invtokens(tokens( ///
        st_local("allindicators"))[ ///
          selectindex(st_matrix("`adj_meas'")[., `j'])]))
    local i`LV`j'' `i`j''
    local ++j
  }
  /* End parsing the adjacency matrices */
  ...
  if ("`structural'" != "") {
    ...
    local j = 1
    foreach var in `alllatents' {
      mata: st_numscalar("`isendo'", sum(st_matrix("`structural'")[., `j']) > 0)
      if (`isendo') {
        mata: st_local("allindep", invtokens(tokens( ///
          st_local("alllatents"))[ ///
            selectindex(st_matrix("`structural'")[., `j'])]))
        local reg3eqs "`reg3eqs'`var'"
        local reg3eqs "`reg3eqs' `allindep'"
        if (`j' == `num_lv') {
          local reg3eqs "`reg3eqs')"
        }
        else {
          local reg3eqs "`reg3eqs') ("
        }
        
        local str_eqn "`var' `allindep'"
        ...
        local dvar : word 1 of `str_eqn'
        local ivars : list str_eqn - dvar
  ...
  if ("`boot'" != "") {
    ...
    * matrix rownames `loadings_reps' = 
    ...
    tempname vcov_load vcov_path
    local tempnamelist "`tempnamelist' `vcov_load' `vcov_path'"
    mata: `vcov_load' = rowsum(`xload_v' :* `annihilate_se', 0)
    if ("`structural'" != "") {
      mata: `vcov_path' = vec(`path_v')
      mata: `vcov_path' = `vcov_path'[selectindex(rownonmissing(`vcov_path'))]
      mata: st_matrix("`vcov_coef'", diag((`vcov_load' \ `vcov_path')))
    }
    else {
      mata: `vcov_path' = J(0, 0, .)
      mata: st_matrix("`vcov_coef'", diag(`vcov_load'))
    }
  }
